...
def app():
    ...
    ##----------------------------------##
    st.title('Custom Search-alyzer')
    st.markdown(    
        '''Search a word in the sidebar using [Twitter's standard search operators](https://developer.twitter.com/en/docs/twitter-api/v1/rules-and-filtering/search-operators "Twitter Search API Docs") to run your own analysis!'''
    )
    ...
    with st.form(key ='form_1'):
        with st.sidebar:
            user_word_entry = st.text_input("1. Enter your keyword query", "kraken", help="Example: `dubas OR toronto OR matthews` returns all tweets having the words dubas, toronto, or matthews (case sensitive)")    
            ...
            team_choice = st.multiselect('2. Filter for specifc NHL team(s)', team_cols, default = 'All', help = 'Replace `All` with other NHL team(s) to compare against the Kraken.')
            mult_choice = st.radio('3. Include tweets about multiple teams?', options = ['Yes', 'No'], help = 'Change to `No` if you only want to see tweets that we think match only a single team')
...
#~-=~-=~-=~-=~-=~-=~-=~-=~-=~-=~-=~-=~-=~-=~-=~-=~--=~-=~-=~-=~-=~-=~-
    ...
    df_tweets, df_new = nf.twitter_search_nhl(user_word_entry ,num_of_tweets)
